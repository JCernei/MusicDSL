...
class TextEditor:
  ...
  def __init__(self,root):
    ...
    self.titlebar = Label(self.top_frame,textvariable=self.title,font=("times new roman",15,"bold"),bd=2,relief=GROOVE)
    self.titlebar.pack(side=RIGHT, fill=BOTH, expand=True)
    self.settitle()

    
    self.play_button = Button(self.top_frame, text ="Play", command = self._play_input)
    self.play_button.pack(side=LEFT)
    # self.stop_button = Button(self.top_frame, text ="Stop", command = self._stop_play)
